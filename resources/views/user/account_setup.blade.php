								...
								<form method="post" action="{{ url('/user/profile-update') }}" enctype="multipart/form-data">
									...
									<div class="process">
										<div class="process-row nav nav-tabs">
											...
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#menuactivities">5</button>
												<p>My Interests</p>
											</div>
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#matchInterest">6</button>
												<p>My Match Interests</p>
											</div>
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#type_of_relationship">7</button>
												<p>Types of Relationship</p>
											</div>
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#horoscope">8</button>
												<p>Horoscope Sign</p>
											</div>
											{{--<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#menu7">9</button>
												<p>Cafe Location</p>
											</div>--}}
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#imageUpload">9</button>
												<p>Image Upload</p>
											</div>
											<div class="process-step">
												<button type="button" disabled class="btn btn-default btn-circle" data-toggle="tab" href="#thankyou">10</button>
												<p>Thank Your</p>
											</div>
										</div>
									</div>
									<div class="tab-content">
										...
										<div  role="tabpanel" id="looking_to_date" class="tab-pane fade">
										...
										<?php /*<div id="where_live_in" class="tab-pane fade">
											...
											<div class="col-md-5 col-sm-6 col-xs-12">
												...
												<select class="form-control" id="liveincity" name="live_in">
													<!-- <option value="">Select State</option>
													@foreach($states as $state)
													<option value="{{ $state->nstate }}" @if(Auth::user()->live_in == $state->zstate) selected @endif>{{ $state->nstate }}</option>
													@endforeach -->
												</select>
											</div>
											@endif
										...
										<div id="menuactivities" class="tab-pane fade">
											...
											<?php
												...
												<div><input type="checkbox" id="{{$activity->id}}" class="act" name="activities" @if(in_array("{{$activity->activity_name}}", $activity_selected)) checked @endif value="{{$activity->activity_name}}"> <label>{{$activity->activity_name}}</label></div>
										...
										<div id="matchInterest" class="tab-pane fade">
											...
											<?php
												...
												<div><input type="checkbox" id="{{$activity->id}}" class="acts" name="matchInterests" @if(in_array("{{$activity->activity_name}}", $matchInterest_selected)) checked @endif value="{{$activity->activity_name}}"> <label>{{$activity->activity_name}}</label></div>
										...
										<div id="type_of_relationship" class="tab-pane fade">
											...
											<div class="col-md-4 col-sm-4 col-xs-12" id="type_of_relationship">
												<label><input type="checkbox" name="relations_type" class="selectall" @if(in_array("newarray", $relations_type)) checked @endif value=""> Select All</label>
										...
										<div id="horoscope" class="tab-pane fade">
											...
											<div class="col-md-5 col-sm-6 col-xs-12">
												...
												<select class="form-control" name="western_sign" disabled>
													<option value="">Choose one</option>
											...
											<div class="col-md-5 col-sm-6 col-xs-12" id="horoscope">
												...
												<select class="form-control" name="chinese_sign" disabled>
													<option value="">Choose one</option>
										...
										<?php /*<div id="menu7" class="tab-pane fade">
											<h4>Cafe Location</h4>
											...
											<input type="hidden" id="cctv" name="cccity" value="">
											...
											<div id="citywise" class="citywse">
												...
												<div class="col-md-6 col-sm-6 col-xs-12">
													...
													<p style="color:red;text-align:left" class="error_mgss"></p>
												</div>
											...
											<!-- <div class="col-md-5 col-sm-6 col-xs-12 zipcode" style="margin-top: 6px;">
										...
										<div id="imageUpload" class="tab-pane fade">
											...
											<ul class="list-unstyled list-inline">
												...
												<li><button type="button" class="btn cafe-submit next-step" data-toggle="tab" href="#thankyou"> Next {{--<i class="fa fa-chevron-right"></i>--}}</button></li>
											</ul>
										</div>
										...
										<div id="thankyou" class="tab-pane fade">
											<div class="account-section">
												<div class="col-md-12">	
													<p class="thank-you-style text-center">Thank you for becoming a member of ifoundyou.</p>
												</div>
											</div>
											<div class="clearfix"></div>
											<ul class="list-unstyled list-inline">
												<li><button type="button" class="btn cafe-submit prev-step" data-toggle="tab" href="#imageUpload">{{--<i class="fa fa-chevron-left"></i>--}} Back</button></li>
												<li><button type="submit" class="btn cafe-submit"><i class="fa fa-check"></i> Submit</button></li>
											</ul>
										</div>
...
<script type="text/javascript">
 ...
 $('.selectall').click(function() {
		...
		var i=0;
		if($(this).is(':checked')){
			$('#type_of_relationship input').prop('checked',true);
			$('#type_of_relationship input').each(function(){
				if($(this).is(':checked')){
					newarray[i] = $(this).val();
				}
				i++;
		...
		{
			$('#type_of_relationship input').prop('checked',false);
			$('#relations_type').text('');
		}
	});
 ...
 $('.looks').click(function() {
		...
		var i=0;
		if($(this).is(':checked')){
			$('#looking_to_date input').prop('checked',true);
			//$('#looking_to_date input').each(function(){
				//if($(this).is(':checked')){
		...
		{
			$('#looking_to_date input').prop('checked',false);
		}
	});
 ...
 $('.act').click(function() {
		...
		if($(this).is(':checked'))
		{
			$('#matchInterest #'+id).prop('checked',true);
		}	
		else
		{
			$('#matchInterest #'+id).prop('checked',false);
		}
			
			$('#matchInterest input:checked').each(function(){
				arr.push($(this).val());
			});
			$('#matchInterest #matchInterests').val(arr.join(", "));
      ...
      jQuery('#cafe-input').keypress(function(){
    	...
    	jQuery('#cityname').change(function(){
     	...
     	jQuery('#cityname').change(function(){
     		var value = $(this).val();
     		if(value != ''){
     			jQuery('.error_mgs').html('');
    		}
     		else
    		{
     			jQuery('.error_mgs').html('Please select city');
				jQuery('#menu7 .next-step').attr('disabled','disabled');
				// jQuery('#menu7 #selectedlocation').val('');
    		}
     	})
 ...
 jQuery('.next-step').on('click',function(e){
      	if(jQuery('#menu7').hasClass('active')){
      		if(continent == 'USA')
      		{
 				...
 				{
 					var id = jQuery(this).attr('href');
 					jQuery('.error_mgs').html('Please Select State');
 					jQuery('.process-step button').removeClass('btn-info').addClass('btn-default');
      		...
      		else
      		{
 				...
 				else 
 				{
 					...
 					{
 						jQuery('.error_mgs').html('');
 						jQuery('.error_mgss').html('Please Choose Location');
 						jQuery('#menu7 .next-step').attr('disabled','disabled');
 						jQuery('.process-step button').removeClass('btn-info').addClass('btn-default');
						jQuery('.process-step [href="#menu7"]').addClass('btn-info').removeClass('btn-default');
		...
		$('#locations').change(function(){
			...
			$.ajax( {
					...
					success:function(selectedlocation){
						if(selectedlocation!=''){
							// $('#selectedlocation').html(selectedlocation[0].store_name+', '+selectedlocation[0].address_line_1);
							$('#selectedlocation').html(selectedlocation[0].address_line_1);
						}
					}
				})
